...
import { updateExtracurriculer } from '@/lib/extracurricular'
import { customRevalidation } from '@/actions/custom-revalidation'
import Image from 'next/image'
import Modal from '@/app/admin/components/modal'

interface EditExtracurricularModalProps {
  extracurricular: IExtracurricular
  accessToken: string
  onClose: () => void
}

const EditExtracurricularModal = ({ extracurricular, accessToken, onClose }: EditExtracurricularModalProps) => {
  const [name, setName] = useState<string>(extracurricular.name)
  const [description, setDescription] = useState<string>(extracurricular.description)
  ...
  const handleUpdateExtracurricular = async () => {
    if (name && description) {
      ...
      try {
        const response = await updateExtracurriculer(extracurricular.id, name, description, accessToken)
  ...
  return (
    <Modal onClose={onClose}>
      <h2 className="text-xl font-bold text-[#202244]">Edit Ekstrakurikuler</h2>
      <input
        value={name}
      ...
      <div className="flex justify-end gap-3">
        ...
        <button
          onClick={handleUpdateExtracurricular}
          disabled={loading}
