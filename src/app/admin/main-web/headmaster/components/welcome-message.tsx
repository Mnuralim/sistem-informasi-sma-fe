...
const WelcomeMessage = ({ welcomeMessageData, accessToken, headmaster }: Props) => {
  const [welcomeMessage, setWelcomeMessage] = useState<string>(welcomeMessageData?.message || '')
  const [headmasterId, setHeadmasterId] = useState<string>(welcomeMessageData?.teacherId || '')
  ...
  const handleSaveMessage = async () => {
    ...
    try {
      const response = await updateWelcomeMessage(
        'headmaster',
        { message: welcomeMessage, teacherId: headmasterId },
        accessToken
      )
      if (!response.ok) {
        ...
        {welcomeMessageData || showForm ? (
          ...
          <div>
            <p className="text-center">
              Sambutan kepala sekolah tidak ditemukan, untuk menambahkan data sambutan klik{' '}
