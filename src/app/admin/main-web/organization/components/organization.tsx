...
import { updateStructuralOrganization } from '@/lib/structural-organization'
...
const Organization = ({ id, imageUrl, accessToken }: Props) => {
  const [structureImage, setStructureImage] = useState<string>(
    imageUrl ? imageUrl : 'https://via.placeholder.com/800x400?text=Struktur+Organisasi'
  )
  ...
  const handleAddImage = async () => {
    ...
    try {
      const response = await updateStructuralOrganization(id, formData, accessToken)
      ...
      setLoading(false)
      setStructureImage(imagePreview!)
      ...
      <div className="bg-white p-5 lg:p-8 rounded-lg shadow-lg space-y-6">
        ...
        <div className="space-y-6">
          <div className="flex justify-between gap-2">
            <button
              className={`px-4 py-2 text-sm lg:text-base ${
                !isAddingNewImage ? 'bg-orange-05 text-white' : 'bg-gray-200'
              } rounded-lg`}
              onClick={() => {
                setNewImage(null)
          ...
          {!isAddingNewImage && (
            <div
              {...getRootProps()}
              className="border-dashed border-2 border-orange-05  p-6 text-center cursor-pointer"
            >
              <input {...getInputProps()} />
            ...
            <div className="bg-gray-100 p-4 rounded-lg">
              ...
              <div className="mt-4 flex justify-end space-x-4">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-orange-05 text-white rounded-lg hover:bg-[#c43824] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange0bg-orange-05"
                  disabled={loading}
                >
