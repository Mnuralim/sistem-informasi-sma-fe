...
import { createFinanceReport } from '@/lib/finance-report'
import { customRevalidation } from '@/actions/custom-revalidation'
import Modal from '@/app/admin/components/modal'

interface AddFinanceReportModalProps {
  accessToken: string
  onClose: () => void
}

const AddFinanceReportModal = ({ accessToken, onClose }: AddFinanceReportModalProps) => {
  ...
  const handleAddReport = async () => {
    if (title && description && file) {
      ...
      try {
        const response = await createFinanceReport(formData, accessToken)
        ...
        toast.error('Gagal menambahkan laporan keuangan')
      } finally {
        setLoading(false)
      }
    }
  ...
  return (
    <Modal onClose={onClose}>
      <h2 className="text-xl font-bold text-[#202244]">Tambah Laporan Keuangan Baru</h2>
      <input
        type="text"
...
export default AddFinanceReportModal
